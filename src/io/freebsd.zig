...
pub const IO = struct {
        ...
        self.submit(
            ...
            struct {
                fn do_operation(op: anytype) AcceptError!socket_t {
                    ...
                    // FreeBSD doesn't support posix.MSG_NOSIGNAL to avoid getting SIGPIPE on
        ...
        self.submit(
            ...
            struct {
                fn do_operation(op: anytype) WriteError!usize {
                    // FreeBSD file IO (posix.pwrite below) is synchronous. fs_sync() is
                    // called explicitly after writing to ensure durability, as O_DSYNC
                    // on FreeBSD does not guarantee metadata-durable writes in all cases.
    ...
    fn open_socket(self: *IO, family: u32, sock_type: u32, protocol: u32) !socket_t {
        ...
        // FreeBSD supports SOCK_CLOEXEC natively (unlike macOS/Darwin).
        _ = try posix.fcntl(fd, posix.F.SETFD, posix.FD_CLOEXEC);
        // FreeBSD doesn't support posix.MSG_NOSIGNAL, uses SO_NOSIGPIPE instead.
    ...
    pub fn open_data_file(
        ...
        // O_DSYNC ensures data is flushed to the device on every write. fs_sync() is also
        // called explicitly for metadata durability across FreeBSD's data-metadata split.
        var flags: posix.O = .{
            .CLOEXEC = true,
        ...
        // FreeBSD supports O_DIRECT for Direct I/O (unlike macOS/Darwin which uses F_NOCACHE).
        // DIRECT must be set before openat().
        switch (purpose) {
            .format, .open => {
            ...
            .inspect => {
                // Read-only open doesn't need Direct I/O.
            },
        }
    ...
    /// FreeBSD's fsync() flushes to the device cache, which is sufficient for durability
    /// (unlike macOS, which requires F_FULLFSYNC).
    fn fs_sync(fd: fd_t) !void {
        try posix.fsync(fd);
    }

    /// Allocates a file contiguously using fallocate().
    /// Uses the FreeBSD fcntl(F_PREALLOCATE) or posix_fallocate().
    fn fs_allocate(fd: fd_t, size: u64) !void {
        log.info("allocating {}...", .{std.fmt.fmtIntSizeBin(size)});

        // FreeBSD has ftruncate to extend the file; fallocate is optional.
        // Fallocate-style preallocation via ftruncate + pwrite to last byte.
